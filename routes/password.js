  ...
  async.waterfall([
    ...
    function(token, done) {
      ...
      User.findOne({ email: req.body.email }, (err, user) => {
        if (err) throw err;
        if (!user) {
          req.flash("error", "That Our Restaurants account doesn't exist.");
    ...
    function(token, user, done) {
      ...
      let mailOptions = {
        ...
        to: user.email,
        subject: "Reset your Our Restaurants Password",
  ...
  async.waterfall([
    ...
    function(user, done) {
      ...
      let mailOptions = {
        from: "webdevtestonly@gmail.com",
        to: user.email,
        subject: "Your Our Restaurants Password has been changed",
        text: "Hi " + user.firstName + ",\n\n" +
